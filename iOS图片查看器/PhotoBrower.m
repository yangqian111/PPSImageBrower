...
-(void)zonmTap:(UITapGestureRecognizer *)zonmTap{
    ...
    if (photoScrollView.zoomScale > 1.0) {
        [photoScrollView setZoomScale:1.0 animated:YES];
    } else {
        CGPoint touchPoint = [zonmTap locationInView:photoScrollView];
        CGFloat newZoomScale = photoScrollView.maximumZoomScale;
        CGFloat xSize = self.frame.size.width / newZoomScale;
        CGFloat ySize = self.frame.size.height / newZoomScale;
        [photoScrollView zoomToRect:CGRectMake(touchPoint.x - xSize / 2, touchPoint.y - ySize / 2, xSize, ySize) animated:YES];
    }
}
